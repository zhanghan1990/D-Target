...
if __name__=='__main__':
    ...
    for i in range(1,11):
        ...
        fiforesult= getResult(fifo)
        sebfresult=getResult(sebf)
        flowresult=getResult(flow)
        fairresult=getResult(fair)

        total[0]+=sebfresult[0]
        total[1]+=fiforesult[0]
        total[2]+=flowresult[0]
        total[3]+=fairresult[0]

        for j in range(0,5):
            bar1[j]+=fairresult[j]/sebfresult[j]
            bar2[j]+=fairresult[j]/fiforesult[j]
            bar3[j]+=fairresult[j]/flowresult[j]
